...
class Settings(BaseSettings):
    ...
    enable_rag: bool = Field(default=False, validation_alias=AliasChoices("ENABLE_RAG"))
    rag_database_url: str = Field(default="", validation_alias=AliasChoices("RAG_DATABASE_URL"))
    rag_top_k: int = Field(default=4, validation_alias=AliasChoices("RAG_TOP_K"))
    enable_rag_writeback: bool = Field(default=False, validation_alias=AliasChoices("ENABLE_RAG_WRITEBACK"))
    rag_writeback_quality_threshold: int = Field(default=70, validation_alias=AliasChoices("RAG_WRITEBACK_QUALITY_THRESHOLD"))
    openai_embedding_model: str = Field(default="text-embedding-3-small", validation_alias=AliasChoices("OPENAI_EMBEDDING_MODEL"))
    embedding_dim: int = Field(default=1536, validation_alias=AliasChoices("EMBEDDING_DIM"))
    enable_conversation_memory: bool = Field(default=False, validation_alias=AliasChoices("ENABLE_CONVERSATION_MEMORY"))
