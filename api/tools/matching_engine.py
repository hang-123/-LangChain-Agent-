"""MatchingEngine Tool — deterministic keyword matching, 0 LLM.
Migrated from api/agents/matching_agent.py."""
...
async def run_matching_engine(state: dict[str, Any]) -> dict[str, Any]:
    """Run the matching engine as a Tool. Returns state update dict."""
    ...
    archetype_detection = dict(state.get("archetype_detection") or {})
    legitimacy_assessment = dict(state.get("legitimacy_assessment") or {})

    if not job_snapshot or not job_requirements:
        return {
            "match_assessment": {},
            "status": "MatchingEngine 未生成匹配分：缺少 JobSnapshot 或岗位要求。",
        }
    ...
    must_have_reqs = [r for r in job_requirements if str(r.get("requirement_level") or "") == "must_have"]
    other_reqs = [r for r in job_requirements if r not in must_have_reqs]
    ...
    for requirement in must_have_reqs + other_reqs:
        matched, matched_refs = _match_requirement(
            requirement, candidate_skills=candidate_skills,
            evidence_text=evidence_text, resume_evidence=resume_evidence,
        )
        ...
        if matched:
            ...
            gap_entry: dict[str, Any] = {
                ...
                "severity": "hard_blocker" if requirement in must_have_reqs else "significant",
                "evidence_refs": [requirement_id] if requirement_id else [],
            }
            if requirement in must_have_reqs:
                gap_entry["description"] = req_name
    ...
    overall_score = round(
        skills_score * 0.45 + experience_score * 0.2 + domain_fit * 0.15
        + education_score * 0.1 + location_score * 0.1
    )
    ...
    if not resume_evidence:
        ...
        risks.append({"title": "缺少 ResumeEvidence，匹配分析已按保守模式降级。", "severity": "high"})

    # Phase 2: Suspicious job awareness
    legit_tier = legitimacy_assessment.get("tier", "")
    if legit_tier == "Suspicious":
        risks.append({
            "title": "⚠️ 该岗位合法性评估为 Suspicious，建议投递前确认岗位仍开放。",
            "severity": "high",
        })
    ...
    reasoning_notes = ["匹配结论仅基于简历显式证据和岗位快照。"]
    if not resume_evidence:
        reasoning_notes.append("缺少 ResumeEvidence，本轮评分采取保守降级。")
